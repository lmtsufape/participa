...
namespace App\Livewire\Trabalhos\Modals;
...
class AtribuirTrabalho extends Component
{
    ...
    {
        $this->trabalho = Trabalho::with(['autor','coautors.user','revisores.user','modalidade','area'])
    ...
    public function addRevisor()
    {
        ...
        DB::transaction(function () use ($data) {
            ...
            $trabalho = Trabalho::lockForUpdate()->findOrFail($data['trabalhoId']);
            ...
            Mail::to($revisor->user->email)->queue(
                new EmailConviteRevisor($revisor->user, $evento, $subject, $evento->email, $trabalho->titulo)
            );
        });
    ...
    public function removerRevisor(int $revisorId)
    {
        ...
        $trabalho = Trabalho::find($this->trabalhoId);
    ...
    public function render()
    {
        ...
        return view('livewire.trabalhos.modals.atribuir-trabalho', compact('opcoes'));
    }
}
