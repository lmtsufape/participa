    ...
    <div class="container">
        ...
        <form id="editarRespostas" action="{{ route('revisor.editar.respostas') }}" method="post"
                                        ...
                                        @if ($pergunta->respostas()->exists() && $pergunta->respostas->first()->opcoes->count())
                                            ...
                                            @foreach ($pergunta->respostas->first()->opcoes->sortBy('id') as $opcao)
                                                <div class="form-check">
                                                    ...
                                                    @else
                                                        <input class="form-check-input" type="radio"
                                                            name="{{ $pergunta->id }}" value="{{ $opcao->titulo }}"
                                                            id="{{ $opcao->id }}">
                                                    @endif
                                            ...
                                            @forelse ($pergunta->respostas as $resposta)
                                                @if ($resposta->revisor != null && $resposta->trabalho != null && $resposta->paragrafo != null)
                                                    @if ($resposta->revisor->user_id == $revisorUser->id && $resposta->trabalho->id == $trabalho->id)
                                                        <p class="card-text">
                                                            <input type="hidden" name="resposta_paragrafo_id[]"
                                                                value="{{ $resposta->paragrafo->id }}">
                                                            <textarea id="resposta{{ $resposta->paragrafo->id }}" type="text"
                                                                class="form-control @error('resposta' . $resposta->paragrafo->id) is-invalid @enderror"
                                                                name="resposta{{ $resposta->paragrafo->id }}" required>
                                                            @if (old('resposta' . $resposta->paragrafo->id) != null)
                                                                {{ old('resposta' . $resposta->paragrafo->id) }}@else{{ $resposta->paragrafo->resposta }}
                                                            @endif
                                                            </textarea>
                                                        </p>
                                                        <div class="col-form-label text-md-left">
                                                            <small>Resposta visível para o autor? (selecione se sim)
                                                            </small><input type="checkbox" name="paragrafo_checkBox[]"
                                                                value="{{ $resposta->paragrafo->id }}"
                                                                {{ $resposta->paragrafo->visibilidade == true ? ' checked' : '' }}>
                                                        </div>
                                                    @endif
                                                @endif
                                            @empty
            ...
            <div class="col-sm-12" style="margin-top: 20px;">
                ...
                <div class="custom-file">
                    <input type="file" class="form-control" data-placeholder="Nenhum arquivo" data-text="Selecionar"
                        data-btnClass="btn-primary-lmts" name="arquivoAvaliacao">
                </div>
        ...
        <div class="d-flex flex-column flex-md-row mt-4 gap-3">
            ...
            @if ($arquivoAvaliacao != null)
                <a class="btn btn-md btn-primary btn-group"
                    href="{{ route('downloadAvaliacao', ['trabalhoId' => $trabalho->id, 'revisorUserId' => $revisorUser->id]) }}">
                    <img class="mr-1" src="{{ asset('img/icons/file-download-solid.svg') }}" style="width:1em">Baixar
                    trabalho corrigido
                </a>
            @endif
            @if ($trabalho->avaliado($revisor->user))
                ...
                @else
                    @if ($trabalho->aprovado === null)
                        <form class="d-none" action="{{ route('coord.evento.avisoCorrecao', $evento->id) }}" method="POST"
                            id="avisoCorrecao">
                            @csrf
                            <input type="hidden" name="trabalhosSelecionados[]" value="{{ $trabalho->id }}">
                        </form>
                        <button class="btn btn-md btn-primary" form="avisoCorrecao" type="submit">Lembrete de envio de
                            versão corrigida do texto</button>
                    @endif
                    <a href="{{ route('trabalho.encaminhar', [$trabalho->id, $revisor]) }}"
