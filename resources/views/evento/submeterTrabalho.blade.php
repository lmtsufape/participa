...
@section('content')
    <style>
        ...
        .is-loading {
            ...
            background-image:
                url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><circle cx='50' cy='50' r='40' stroke='%23adb5bd' stroke-width='10' fill='none' stroke-linecap='round'><animateTransform attributeName='transform' type='rotate' dur='1s' repeatCount='indefinite' from='0 50 50' to='360 50 50'/></circle></svg>");
        }
                        ...
                        <div class="card card-body">
                                ...
                                @if ($indice == "etiquetaareatrabalho")
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <select id="modalidade" class="form-select  @error('modalidade_id') is-invalid @enderror"
                                                disabled
                                            >
                                                @foreach($modalidades as $m)
                                                    ...
                                                    @endphp
                                                    <option value="{{ $m->id }}" @selected($m->id === $modalidade->id)>
                                                        {{ $label }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="row justify-content-center">
                                        ...
                                        @if ($modalidade->arquivo == true)
                                            <div class="col-sm-12" style="margin-top: 20px;">
                                                <h6 class="">
                                                    <strong>{{$formSubTraba->etiquetauploadtrabalho}}</strong>
                                                </h6>

                                                @if($modalidade->submissaoUnica == true)
                                                    <div class="alert alert-warning w-100">
                                                        <strong>ATENÇÃO:</strong> Nesta modalidade só é possível submeter o trabalho apenas uma vez.
                                                    </div>
                                                @endif
                                                <div class="">
                                                    <label for="arquivo"
                                                        style="display:inline-flex; align-items:center; gap:8px;
                                                            background-color:#D44100; color:white; border-radius:10px;
                                                            padding:8px 16px; cursor:pointer;"
                                                        onmouseover="this.style.backgroundColor='#B73A00';"
                                                        onmouseout="this.style.backgroundColor='#D44100';"
                                                        onmousedown="this.style.backgroundColor='#9F2F00';"
                                                        onmouseup="this.style.backgroundColor='#B73A00';"
                                                    >
                                                        <img src="{{ asset('img/icons/upload.svg') }}"
                                                        class="upload-icon"
                                                        alt="ícone de upload"
                                                        width="24"
                                                    ...
                                                    <input type="file" id="arquivo" class="@error('arquivo') is-invalid @enderror" name="arquivo" style="position:absolute; left:-9999px;">
                                                    ...
                                                    @enderror
                                                    <span id="nome-arquivo" style="margin-left:10px; vertical-align:middle;"></span>
                                                </div>

                                                <small>
                                                    <strong>Extensão de arquivos aceitas:</strong>
                                                    <span id="extensoes-aceitas">
                                                        @foreach ($modalidade->tiposAceitos() as $tipo)
                                                            {{ $tipo }};
                                                        @endforeach
                                                    </span>
                                                    <br>
                            ...
                            @foreach ($ordemCampos as $indice)
                                @if($indice == "etiquetacoautortrabalho")
                                    <div style="margin-top:20px">
                                        {{-- Dados do autor --}}
                                        <h5>{{$evento->formSubTrab->etiquetaautortrabalho}}</h5>
                                        <div class="card card-body mb-4">
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <label for="autor_email">E-mail</label>
                                                      <input type="text" name="autor[email]" id="autor_email" class="form-control @error('autor.email') is-invalid @enderror" value="{{ old('autor.email', auth()->user()->email) }}" @if(!(auth()->user()->administrador()->exists() || auth()->user()->coordComissaoCientifica()->where('eventos_id', $evento->id)->exists() || $evento->coordenadorId == auth()->user()->id)) readonly value="{{auth()->user()->email}}"@endif>
                                                ...
                                                <div class="col-md-6">
                                                    ...
                                                    <input type="text" name="autor[nome]" id="autor_nome" class="form-control @error('autor.nome') is-invalid @enderror" value="{{ old('autor.nome', auth()->user()->name) }}" @if(!(auth()->user()->administrador()->exists() || auth()->user()->coordComissaoCientifica()->where('eventos_id', $evento->id)->exists() || $evento->coordenadorId == auth()->user()->id)) readonly value="{{auth()->user()->name}}"@endif>
                                                    @error('autor.nome')
                                                        <span class="invalid-feedback" role="alert">
                                                            <strong>{{ $message }}</strong>
                                                        </span>
                                        ...
                                        @if(in_array('etiquetacoautortrabalho', $ordemCampos))
                                            <button id="btn-add-coautor" type="button" class="btn btn-primary btn-padding border mb-2 float-end"
                                                    style="text-decoration: none; border-radius: 10px; background-color: #D44100"
                                            ...
                                            <div class="toast-container position-fixed bottom-0 end-0 p-3">
                                                <div id="toast-limite" class="toast bg-danger text-white" role="alert">
                                                    <div class="toast-body">
                                                        Limite máximo de coautores atingido.
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div id="coautores-wrapper" class="d-none">
                                            <h5>{{$evento->formSubTrab->etiquetacoautortrabalho}}</h5>
                                            <div id="coautores-list" class="mt-3">
                                                @foreach (old('coautores', []) as $i => $coautor)
                                                    <div class="item card w-100 mb-2" data-index="{{ $i }}">
                                                        <div class="row card-body">
                                                            <div class="col-md-6">
                                                                <label for="coautores_{{$i}}_email">E-mail</label>
                                                                <input type="email" class="form-control @error("coautores.$i.email") is-invalid @enderror" id="coautores_{{$i}}_email" name="coautores[{{ $i }}][email]" value="{{ old("coautores.$i.email") }}" placeholder="E-mail" value="" required>
                                                                @error("coautores.$i.email")
                                                                    <span class="invalid-feedback" role="alert">
                                                                        <strong>{{ $message }}</strong>
                                                                    </span>
                                                                @enderror
                                                            ...
                                                            <div class="col-md-6">
                                                                <label for="coautores_{{$i}}_nome">Nome Completo</label>
                                                                <input type="text" class="form-control @error("coautores.$i.nome") is-invalid @enderror" id="coautores_{{$i}}_nome" name="coautores[{{ $i }}][nome]" value="{{ old("coautores.$i.nome")  }}" placeholder="Nome" value="" required>
                                                                @error("coautores.$i.nome")
                                                                    <span class="invalid-feedback" role="alert">
                                                                        <strong>{{ $message }}</strong>
                                                                    </span>
                                                                @enderror
                                                            ...
                                                            <div class="col-md-4 col-lg-3 justify-content-center d-flex align-items-end btn-group pb-1">
                                                                <button type="button" class="btn text-danger btn-remover"><img src="{{asset('img/icons/trash-alt-regular.svg')}}" class="icon-card" width="24" alt="Remover"></button>
                                                                <button type="button" class="btn btn-link btn-sobe"><img src="{{asset('img/icons/sobe.png')}}" class="icon-card" width="24" alt="Subir"></button>
                                                                <button type="button" class="btn btn-link btn-desce"><img src="{{asset('img/icons/desce.png')}}" class="icon-card" width="24" alt="Descer"></button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                @endforeach
                                            </div>
                                            <template id="tpl-coautor">
                                                <div class="item card w-100 mb-2" data-index="__INDEX__">
                                                    <div class="row card-body">
                                                        <div class="col-md-6">
                                                            <label for="coautores__INDEX__email">E-mail</label>
                                                            <input type="email" class="form-control" name="coautores[__INDEX__][email]" id="coautores__INDEX__email" placeholder="E-mail" required>
                                                        </div>
                                                        <div class="col-md-6">
                                                            <label for="coautores__INDEX__nome">Nome Completo</label>
                                                            <input type="text" class="form-control" name="coautores[__INDEX__][nome]" id="coautores__INDEX__nome" placeholder="Nome" required>
                                                        </div>
                                                        <div class="col-md-4 col-lg-3 justify-content-center d-flex align-items-end btn-group pb-1">
                                                            <button type="button" class="btn text-danger btn-remover"><img src="{{asset('img/icons/trash-alt-regular.svg')}}" class="icon-card" width="24" alt="Remover"></button>
                                                            <button type="button" class="btn btn-link btn-sobe"><img src="{{asset('img/icons/sobe.png')}}" class="icon-card" width="24" alt="Subir"></button>
                            ...
                            <div class="d-flex justify-content-between pt-5">
                                <a href="{{route('evento.visualizar',['id'=>$evento->id])}}"
                                    class="btn btn-secondary w-25">{{__('Cancelar')}}
                                </a>

                                <button type="submit" class="btn btn-primary w-25" style="background-color: #034652; color: white; border-color: #034652;">
                                    {{ __('Enviar') }}
                                </button>
                            </div>
                        </div>
...
<script>
    $(document).ready(function () {
        ...
        $('#nome-arquivo').text(this.files.length ? this.files[0].name : '');
            });
        const $wrapper = $('#coautores-wrapper');
        const $lista = $('#coautores-list');
        const $tpl  = $('#tpl-coautor').html(); // pega o conteúdo do <template>
        const $btnAdd = $('#btn-add-coautor');

        // Se quiser limitar
        const max = {{ (int) ($modalidade->numMaxCoautores ?? 0) }};
        let index = $lista.find('.item').length;
        ...
        function atualizarVisibilidade() {
            const qtd = $lista.find('.item').length;
            if (qtd === 0) {
                $wrapper.addClass('d-none'); // esconde
            } else {
                $wrapper.removeClass('d-none'); // mostra
            }
        }
        // Atualiza contador (opcional)
        function atualizarContador() {
            const qtd = $lista.find('.item').length;
            $btnAdd.prop('disabled', max > 0 && qtd >= max);
        ...
        $btnAdd.on('click', function () {
            const qtd = $lista.find('.item').length;
            if (max > 0 && qtd >= max){
                const toast = new bootstrap.Toast($('#toast-limite'));
                toast.show();
        ...
        // Remover coautor
        $lista.on('click', '.btn-remover', function () {
        ...
        // Subir coautor
        $lista.on('click', '.btn-sobe', function () {
        ...
        // Descer coautor
        $lista.on('click', '.btn-desce', function () {
            const $item = $(this).closest('.item');
            $item.next('.item').after($item);
            reindexar();
        });
        // dispara quando QUALQUER email de coautor perde o foco (delegado no container)
        $(document).on('blur', 'input[name$="[email]"]', function () {
            const $email = $(this);
            const nameAttr = $email.attr('name'); // ex: autor[email] ou coautores[0][email]
            ...
            let $nome;
            if (nameAttr.startsWith('coautores')) {
                // dentro da mesma linha de coautor
                $nome = $email.closest('.item').find('input[name$="[nome]"]');
            } else if (nameAttr.startsWith('autor')) {
                // campo do autor principal
                $nome = $('input[name="autor[nome]"]');
            }
            // feedback leve
            $email.addClass('is-loading').removeClass('is-valid is-invalid');
        ...
        // Reindexar os nomes dos inputs
        function reindexar() {
            $lista.find('.item').each(function (i) {
                $(this).attr('data-index', i);
                $(this).find('input[name^="coautores"]').each(function () {
                    const name = $(this).attr('name');
                    const novo = name.replace(/\[\d+\]/, '[' + i + ']');
                    $(this).attr('name', novo);
                });
            });
            index = $lista.find('.item').length;
        }
